...
bool SDLHandler::init(){
	...
	Logger::getInstance().log(string(SDLHANDLER_NONINITSDL_MSG)+string(SDL_GetError()));
        return false;
    }
    if(!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)){
        //printf("ERROR: SDL_image could not initialize! SDL_image Error: %s", IMG_GetError());
	Logger::getInstance().log(string(SDLHANDLER_NONINITIMG_MSG)+string(IMG_GetError()));
